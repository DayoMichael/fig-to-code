...
export async function runWorkerLoop(options: WorkerPollOptions = {}): Promise<never> {
  const intervalMs = options.pollIntervalMs ?? Number(process.env.WORKER_POLL_MS ?? 2000);
  const apiBase = options.apiBase ?? process.env.API_BASE ?? "http://localhost:3000";

  console.log(`Fig2Code worker polling ${apiBase} every ${intervalMs}ms`);

  for (;;) {
    try {
      await pollAndProcessOnce({ ...options, apiBase });
    } catch (error) {
      console.error(
        "Worker poll error:",
